...
public class ArtifactManager {
	...
	public ArtifactManager(String name, String version, String jenkinsURLBase,
		...
		buildAPIString = "/api/xml?tree=jobs[builds[number,artifacts[fileName],actions[levelValue]]]&wrapper=jenkins&xpath=//job/build[starts-with(artifact/fileName/text(),'"
				+ name.toLowerCase()
				+ "-"
				+ version
				+ "')]&exclude=//job/build/action[not(node())]|//job/build[not(artifact)]";
		recommendedAPIString = buildAPIString
				+ "|//job/build[not(action/levelValue=4)]";
	}
	...
	public void setup(boolean printProgress, int progressMin, int progressMax) {
		...
		Object artifactAPIResponse;
		Object recommendedArtifactAPIResponse;
		YamlConfiguration database = YamlConfiguration
				.loadConfiguration(SpaceModule.DATABASE);
		ConfigurationFile allBuilds;
		ConfigurationFile recommendedBuilds;
		try {
			artifactAPIResponse = SpaceModule.getXStream().fromXML(
					Utilities.getContent(jenkinsURLBase + buildAPIString));
			recommendedArtifactAPIResponse = SpaceModule.getXStream()
					.fromXML(
							Utilities.getContent(jenkinsURLBase
									+ recommendedAPIString));
		} catch (NullPointerException e) {
			Console.progress(
					"Failed to connect to update server, skipping update check.",
					progressMax);
			return;
		}
		allBuilds = new ConfigurationFile(new Node(
				artifactAPIResponse));
		recommendedBuilds = new ConfigurationFile(new Node(
				recommendedArtifactAPIResponse));
		...
		String lastRecommendedBuild = recommendedBuilds
				.getString("jenkins[0].number");
		developmentBuild = Integer.parseInt(lastBuild.trim());
		try {
			if (lastRecommendedBuild != null)
				recommendedBuild = Integer
						.parseInt(lastRecommendedBuild.trim());
			else
		...
		if (lastCheckedBuild == developmentBuild) {// No need to update the
													// artifact name
			artifactName = database.getString(name + "-" + version
					+ ".artifactName");
		} else {
			artifactName = (SpaceModule.getInstance().recommended ? recommendedBuilds
					: allBuilds).getString("jenkins[0].artifact.fileName");
			database.set(name + ".artifactName", artifactName);
		}

		// Map build artifacts to hashes taken from jenkins
		double progressDiv = (double) (progressMax - progressMin)
				/ (double) allBuilds.getList("jenkins").size();
		for (Object o : (recommended ? recommendedBuilds : allBuilds)
				.getList("jenkins")) {
			ConfigurationFile c = new ConfigurationFile(new Node(o));
			int number = Integer.parseInt(c.getString("number"));

			final String buildPage = Utilities.getContent(jenkinsURLBase
					+ "/job/" + name + "/" + number + "/artifact/target/"
					+ artifactName + "/*fingerprint*/");

			if (buildPage != null) {
				final int beginIndex = buildPage
						.indexOf("<div class=\"md5sum\">MD5: ") + 25;
				final String md5 = buildPage.substring(beginIndex,
						beginIndex + 32);
